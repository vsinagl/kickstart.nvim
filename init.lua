--WARNING:
--HERE START THE SETTINGS.LUA FILE
...
vim.opt.expandtab = false --dont put spaces instead of tabs
vim.opt.tabstop = 4
...
-- Exit terminal mode in the bkiltin terminal with a shortcut that is a bit easier
...
--WARN: zde zacina sekce pro pluginy !!
--
...
require('lazy').setup({
  ...
  { -- LSP Configuration & Plugins
    ...
    config = function()
      ...
      -- NOTE: ADDING LANGUAGE SERVER PROTOCOLS:
      -- into table servers we can add language server protocols that we want to acutally use.
      -- they will be installed by MASON plugin
      --  we need just to uncomment the lsp server or add the name of the server that we want to use and that's it
      --
      local servers = {
        clangd = {},
        -- gopls = {},
        pylsp = {},
        -- pyright = {}, -- not working, some fucking problem
        -- rust_analyzer = {},
...
--
...

  {
    ...
    config = function()
      vim.keymap.set('n', '<leader>e', ':NvimTreeToggle<CR>', { desc = 'show tree' })
...

  {
    ...
    init = function()
      ...
      --NOTE: keymap settings for buffers
      vim.keymap.set('n', '<leader>x', ':BufferClose<CR>', { desc = 'go to next buffer' })
      require('barbar').setup()
    end,
  },

  --NOTE: telescope fuzzy finder for themes. Can be added also to telescope config!
  --
  {
